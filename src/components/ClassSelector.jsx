      ...
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto">
          <span className="text-sm font-bold text-gray-600 uppercase tracking-wider flex items-center gap-2">
            <GraduationCap size={18} className="text-indigo-600" aria-hidden="true" /> Klass:
          </span>
          {data.classes.map(c => (
            <button
              key={c.id}
              onClick={() => selectClass(c.id)}
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 whitespace-nowrap active-press ${
                currentClassId === c.id
